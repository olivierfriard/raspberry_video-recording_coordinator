...
def get_raspberrypi_model():
    """
    get the model of Raspberry Pi using /proc/device-tree/model
    """
    try:
        process = subprocess.run(["cat", "/proc/device-tree/model"], stdout=subprocess.PIPE)
...
def get_wifi_ssid():
    """
    get name of current wifi access point
    """
...
def get_free_space():
    ...
    """
    stat = shutil.disk_usage(f"/home/{os.environ['USER']}")
...
@app.route("/")
def home():
    return f"""<h1>Raspberry Pi<b>{socket.gethostname()}</b></h1>
video control server v. {__version__}<br>
<br>
Model: {get_raspberrypi_model()}<br>
hostname: {socket.gethostname()}<br>
IP address: {get_ip()}<br>
Wi-Fi MAC Addr: {get_hw_addr(WIFI_INTERFACE)}<br>
...
def status():
    ...
    try:
        ...
        server_info = {
            "status": "OK",
            "Raspberry Pi model": get_raspberrypi_model(),
...
class Raspivid_thread(threading.Thread):
    def __init__(self, duration, w, h, fps, quality, prefix):
        ...
        logging.info(
            "thread args: {} fps: {} resolution: {}x{} quality: {} prefix:{}".format(
                duration, fps, w, h, quality, prefix
            )
        )
    ...
    def run(self):
        ...
        cmd = (
            ...
            + "-o {VIDEO_ARCHIVE}/{hostname}_{prefix}_{file_name}.h264".format(
                VIDEO_ARCHIVE=VIDEO_ARCHIVE, file_name=file_name, prefix=self.prefix, hostname=socket.gethostname()
            )
        )
...
def sync_time(date, hour):
    ...
    completed = subprocess.run(
        ["sudo", "timedatectl", "set-time", "{date} {hour}".format(date=date, hour=hour)]
    )  # 2015-11-23 10:11:22
    if completed.returncode:
        return str(
            {
                "result": "time not synchronised",
